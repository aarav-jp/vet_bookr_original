...
class _PetTrainersPageState extends State<PetTrainersPage>
  ...
  static const String _kLocationServicesDisabledMessage =
      'Phone location is turned off. Please turn it on to view nearby pet trainers.';
  ...
  Future<void> _showLocationOffDialog() async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Please turn on your phone location to find nearby pet trainers.',
        ),
        actions: [
          TextButton(
            onPressed: () {
  ...
  Future<void> getTotalData() async {
    ...
    try {
      ...
      final String vetsUrl =
          "https://maps.googleapis.com/maps/api/place/nearbysearch/json?keyword=pet+trainers&location=${latLng[0]},${latLng[1]}&radius=$apiChanger&type=pet_trainer&key=${Constants.apiKey}";
      ...
      if (response.statusCode != 200) {
        throw Exception('Failed to load pet trainers. Server error ${response.statusCode}.');
      }
      ...
      if (data["results"] == null || data["results"] is! List) {
        throw Exception('No pet trainers found right now.');
      }
      ...
      setState(() {
        ...
        errorMessage = loadedClinics.isEmpty
            ? 'No pet trainer found in this area.'
            : null;
      });
    } on TimeoutException {
      if (!mounted) return;
