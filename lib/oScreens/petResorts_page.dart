...
class _PetResortsPageState extends State<PetResortsPage>
  ...
  static const String _kLocationServicesDisabledMessage =
      'Phone location is turned off. Please turn it on to view nearby pet-friendly resorts.';
  ...
  Future<void> _showLocationOffDialog() async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Please turn on your phone location to find nearby pet-friendly resorts',
        ),
        actions: [
          TextButton(
            onPressed: () {
  ...
  Future<void> getTotalData() async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Failed to load resorts. Server error ${response.statusCode}.');
      }
      ...
      if (data["results"] == null || data["results"] is! List) {
        throw Exception('No pet-friendly resorts found right now.');
      }
      ...
      setState(() {
        ...
        errorMessage = loadedClinics.isEmpty
            ? 'No pet-friendly resorts found in this area.'
            : null;
      });
    } on TimeoutException {
      if (!mounted) return;
