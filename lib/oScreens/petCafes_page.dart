...
class _PetCafesPageState extends State<PetCafesPage>
  ...
  static const String _kLocationServicesDisabledMessage =
      'Phone location is turned off. Please turn it on to view nearby pet-friendly cafes.';
  ...
  Future<void> _showLocationOffDialog() async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Please turn on your phone location to find nearby pet-friendly cafes.',
        ),
        actions: [
          TextButton(
            onPressed: () {
  ...
  Future<void> getTotalData() async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Failed to load pet-friendly cafes. Server error ${response.statusCode}.');
      }
      ...
      if (data["results"] == null || data["results"] is! List) {
        throw Exception('No pet-friendly cafes found right now.');
      }
      ...
      setState(() {
        ...
        errorMessage = loadedClinics.isEmpty
            ? 'No pet-friendly cafes found in this area.'
            : null;
      });
    } on TimeoutException {
      if (!mounted) return;
