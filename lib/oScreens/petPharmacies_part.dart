...
class _PetPharmaciesPageState extends State<PetPharmaciesPage>
  ...
  static const String _kLocationServicesDisabledMessage =
      'Phone location is turned off. Please turn it on to view nearby vet clinics.';
  ...
  Future<void> _showLocationOffDialog() async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Please turn on your phone location to find nearby vet clinics.',
        ),
        actions: [
          TextButton(
            onPressed: () {
  ...
  Future<void> getTotalData() async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Failed to load pharmacies. Server error ${response.statusCode}.');
      }
      ...
      if (data["results"] == null || data["results"] is! List) {
        throw Exception('No clinics found right now.');
      }
